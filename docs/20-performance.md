...
Typical times for different log sizes:

| Log Size | Events | Time | Memory |
|----------|--------|------|--------|
| Small | 1K | 10ms | 10MB |
| Medium | 10K | 50ms | 50MB |
| Large | 100K | 500ms | 200MB |
| XL | 1M | 5s | 1GB |
| XXL | 10M+ | 50s+ | 10GB+ |
...
Each line matched against up to 209 parsers (short-circuits on first match), then format-specific parsing.
