...
class ApacheAccessParser(LogParser):
    FORMAT_NAME = "apache-access"

    @classmethod
    def can_parse(cls, path: Path | None, sample_lines: list[str]) -> bool:
        if path and ("apache" in str(path) or "httpd" in str(path)) and "access" in path.name:
            return True
        # Don't claim nginx paths via content detection — the CLF format is
        # shared with nginx and nginx_access.py owns those paths.
        if path and "nginx" in str(path):
            return False
