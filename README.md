...
![Parsers](https://img.shields.io/badge/log%20formats-209-blue)
![Wheel size](https://img.shields.io/badge/wheel-35MB-lightgrey)
...
Covers 209 formats: Linux/Unix system logs plus web servers, databases,
...
### 209 Log Format Parsers by Category

Real counts, pulled directly from the [complete parser index](docs/06-supported-log-types.md#complete-parser-index) — every category total below is a count of actual parser files, not an estimate.

```mermaid
pie showData
    title 209 Parsers by Category
    "Security / SIEM (19)" : 19
    "Identity & Access Mgmt (5)" : 5
    "Application Frameworks (18)" : 18
    "Databases (17)" : 17
    "Containers & Cloud-Native (17)" : 17
    "Infrastructure, HA & DNS (17)" : 17
    "Web Servers & Proxies (16)" : 16
    "Observability / Agents (16)" : 16
    "Linux Daemons (16)" : 16
    "Other Daemons (11)" : 11
    "Storage & Virtualization (10)" : 10
    "System & Auth (12)" : 12
    "Mail (8)" : 8
    "Network / VPN (8)" : 8
    "Build & Package Mgmt (8)" : 8
    "Message Queues (6)" : 6
    "Big Data (4)" : 4
    "Fallback (1)" : 1
```

See [docs/06-supported-log-types.md](docs/06-supported-log-types.md) for the full breakdown and a complete alphabetical index of all 209 parsers.

---

## Features

- **209 log format parsers** with automatic format detection
...
# Or install as a standalone CLI tool with uv
...
Requires **Python 3.11+**. AI models are embedded — no separate download needed. `uv`/`uvx` work out of the box since logcrux ships a standard `[project.scripts]` entry point; no special packaging is needed for either.
...
**209 formats total** — Below are representative examples from each category. Use `logcrux --help` or check [docs/06-supported-log-types.md](docs/06-supported-log-types.md) for the complete list.
...
| Memory | ~100 KB per 10,000 events |
| Model load | ~2 s on first run (cached) |

For logs > 100 K events use `--last` to limit the time window.
